...
1. Use the repository root [`render.yaml`](render.yaml) as the Render service definition.
2. Connect the GitHub repository to Render so pushes to `main` trigger deploys automatically.
3. Configure backend environment variables in the Render dashboard using the same contract documented in [`deploy/render-backend.md`](deploy/render-backend.md).
4. Verify the service at `/health/live` and `/docs` after deploy.
