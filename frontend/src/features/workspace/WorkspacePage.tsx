...
const Workspace: React.FC = () => {
  ...
  const {
    ...
    id,
    useCallback((restored: WorkspaceRestoreState) => {
      setReportTopic(restored.reportTopic);
      setChatPaperIds(restored.chatPaperIds);
      setFaultPaperId(restored.faultPaperId);
      setSelectedPaperId(restored.selectedPaperId);
  ...
  const removePaperFromWorkspace = async (paper: Paper) => {
    if (!workspace || !paper?.id) return;
    const confirmed = window.confirm(
      `Remove "${paper.title}" from this workspace? This cannot be undone.`
    );
    if (!confirmed) return;
    ...
    setError(null);
    try {
      await api.delete(`/workspaces/${workspace.id}/papers/${paper.id}`);
      setChatPaperIds((prev) => prev.filter((idValue) => idValue !== paper.id));
      setFaultPaperId((prev) => (prev === paper.id ? null : prev));
      setSelectedPaperId((prev) => (prev === paper.id ? null : prev));
      setPaperCheckCache((prev) => {
        const next = { ...prev };
        delete next[paper.id];
        return next;
      });
      setPaperExplainCache((prev) => {
        const next = { ...prev };
        delete next[paper.id];
        return next;
      });
      setCitationCache((prev) => {
        ...
        Object.keys(next).forEach((key) => {
          if (key.startsWith(`${paper.id}:`)) {
            delete next[key];
          }
        });
        return next;
      });
      toastSuccess('Paper removed from workspace.');
      await loadWorkspace();
    } catch (err: unknown) {
      const message = apiErrorMessage(err, 'Failed to remove paper from workspace.');
      setError(message);
      toastError(message);
    } finally {
                          ...
                          <>
                            ...
                            <div className="mt-4 flex flex-wrap gap-2">
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  void removePaperFromWorkspace(selectedPaper);
                                }}
                                disabled={deletingPaperId !== null}
                                className="inline-flex items-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700 disabled:cursor-not-allowed disabled:opacity-55"
                              >
                                {deletingPaperId === selectedPaper.id ? (
                                  <Loader2 className="h-4 w-4 animate-spin" />
                                ) : (
