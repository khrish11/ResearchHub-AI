...
const AGENT_SELECTIONS_KEY = 'researchhub.research_agent.paper_selection.v1';
const AGENT_CHAT_KEY = 'researchhub.research_agent.chat_history.v1';

const getAgentSelections = (): Record<string, number[]> => {
  try {
...
const saveAgentSelections = (value: Record<string, number[]>) => {
  localStorage.setItem(AGENT_SELECTIONS_KEY, JSON.stringify(value));
};

const getAgentChatHistory = (): Record<string, ChatMessage[]> => {
  try {
...
const saveAgentChatHistory = (value: Record<string, ChatMessage[]>) => {
...
const qualityTone = (label: string): 'green' | 'amber' | 'rose' | 'indigo' | 'slate' => {
...
const ResearchAgent: React.FC = () => {
  ...
  }, [resumeSelectedPaperIds, selectedWorkspace]);
