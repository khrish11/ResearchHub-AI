...
interface WorkspaceInsightItem {
  text: string;
  source_refs: number[];
}

interface WorkspaceInsightSource {
  source_index: number;
  source_id: string;
  source_type: string;
  title: string;
...
interface WorkspaceInsightsPayload {
...
interface WorkspaceFeedResponse {
  status: string;
  workspace_id: number;
  disclaimer: string;
...
interface OnboardingDemoSamplePaper {
  title: string;
  authors: string;
}

interface OnboardingDemoFeedPreview {
  type: string;
  title: string;
  description: string;
  importance_score: number;
...
interface DemoModeStateResponse {
  ...
  const fetchWorkspaceInsights = async (workspaceId: number, refresh = false) => {
    ...
    try {
      const response = refresh
        ? await api.post<WorkspaceInsightsResponse>(`/workspace-insights/${workspaceId}/refresh?run_inline=true`)
        : await api.get<WorkspaceInsightsResponse>(`/workspace-insights/${workspaceId}?run_inline=true`);
